...
public class LinkedTaskList  extends TaskList {
    private static final Logger LOGGER = Logger.getLogger(LinkedTaskList.class.getName());
    private class LinkedTaskListIterator implements Iterator<Task>{
        ...
        public Task next() {
            if (!hasNext()){
                LOGGER.error("next iterator element doesn't exist");
    ...
    public boolean remove(Task task) {
        if (isNull(task)) {
            LOGGER.error("removing task that doesn't exist");
            throw new IllegalArgumentException("Task is null");
        }

        Node cursor = last;
        if (last.getTask().equals(task))
            this.last = last.getLast();
