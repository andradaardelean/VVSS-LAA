...
public class MainController {
    ...
    @FXML
    public void deleteTask() throws Exceptions {
    ...
    public void showFilteredTasks(){
        if(!isValidDate(datePickerFrom.getValue()) || !isValidTime(fieldTimeFrom.getText()) || !isValidDate(datePickerTo.getValue()) || !isValidTime(fieldTimeTo.getText())){
            showAlert("Please provide valid values!");
        }else {
    ...
    public static boolean isValidTime(String time) {
        return !time.isEmpty() && time.matches("\\d{2}:\\d{2}");
    }
